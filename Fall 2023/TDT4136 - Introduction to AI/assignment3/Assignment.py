...
class CSP:
    ...
    def backtrack(self, assignment):
        ...
        if all([len(assignment[key]) == 1 for key in assignment]): return assignment
    ...
    def select_unassigned_variable(self, assignment):
        ...
        """
        return next((key for key in assignment if len(assignment[key]) > 1), None)
    ...
    def revise(self, assignment, i, j):
        ...
        """
        if len(assignment[j]) != 1: return False
        y = assignment[j][0]
        if y in assignment[i]:
            assignment[i].remove(y)
            return True
        return False
